...
function StudentFundingPage() {
  ...
  // Filter states
  const [filters, setFilters] = useState(DEFAULT_FILTERS.STUDENT_FUNDING || {
    studentName: '',
    countryAppliedFor: '',
    institutionName: '',
    status: '',
  ...
  const [columnVisibility, setColumnVisibility] = useState(() => {
    const savedVisibility = localStorage.getItem('studentFundingColumnVisibility');
  ...
  useEffect(() => {
    localStorage.setItem('studentFundingColumnVisibility', JSON.stringify(columnVisibility));
  }, [columnVisibility]);

  // Fetch data from API
  const fetchData = async () => {
    if (!user || !user._id) {
      console.log('Agent Student Funding - User not authenticated');
      setLoading(false);
      return;
    }
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `student_funding_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleRowClick = (params) => {
    navigate(`/agent/student-funding/view/${params.row.id}`);
  };
          ...
          <Stack direction="row" alignItems="center" spacing={2}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography variant="h4" sx={{ color: 'white', fontWeight: 'bold', mb: 0.5 }}>
                Student Funding Management
              </Typography>
              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.9)' }}>
                Manage student funding requests and track their progress
              </Typography>
            </Box>
            <Chip
...
export default StudentFundingPage;

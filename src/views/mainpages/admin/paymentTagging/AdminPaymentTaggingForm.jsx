// Admin Payment Tagging Form - similar to Agent but with Agent selection
...
function AdminPaymentTaggingForm() {
    ...
    const fetchAgents = async () => {
      try {
        ...
        if (response.data.users) {
          const activeAgents = response.data.users.filter(
  ...
  useEffect(() => {
    if (formData.agentRef) {
      const filtered = students.filter(
        (student) => student.agentCode === formData.agentRef
      );
      setFilteredStudents(filtered);
    } else {
      setFilteredStudents([]);
    }
  }, [formData.agentRef, students]);
  ...
  const handleChange = (e) => {
    ...
    // Reset studentRef when agentRef changes
    if (name === 'agentRef') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Upload files if new files are added
      ...
      if (isEditMode) {
        response = await axios.put(
          `https://abroad-backend-gray.vercel.app/api/payment-tagging/admin/update/${id}`,
          submitData,
          { withCredentials: true }
        );
      } else {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          navigate(`/admin/payment-tagging/view/${response.data.data._id}`);
        }, 1500);
      }
    } catch (error) {
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <Stack direction="row" spacing={2} justifyContent="flex-end">
                    <Button
                      variant="outlined"
                      onClick={() => navigate('/admin/payment-tagging')}
                    >
                      Cancel
                    </Button>
                    <Button
...
export default AdminPaymentTaggingForm;

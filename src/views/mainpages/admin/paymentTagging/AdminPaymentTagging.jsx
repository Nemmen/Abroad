...
const allColumns = [
  { field: 'agentRef', headerName: 'Agent', width: 150 },
...
const AdminPaymentTagging = () => {
  ...
  const [filters, setFilters] = useState(() =>
    loadFiltersFromStorage(
      FILTER_STORAGE_KEYS.ADMIN_PAYMENT_TAGGING,
      DEFAULT_FILTERS.ADMIN_PAYMENT_TAGGING || {
        agentRef: 'all',
        letterType: 'all',
        status: 'all',
        searchTerm: '',
  ...
  const handleRowClick = (params) => {
    navigate(`/admin/payment-tagging/view/${params.id}`);
  };
    ...
    const fetchAgents = async () => {
      try {
        const response = await axios.get(
          'https://abroad-backend-gray.vercel.app/auth/getAllusers'
        );
        if (response.data.data) {
          const activeAgents = response.data.data.filter(
            (agent) => agent.userStatus === 'active'
          );
  ...
  useEffect(() => {
    const fetchPaymentTagging = async () => {
      ...
      try {
        ...
        if (filters.agentRef && filters.agentRef !== 'all') {
          queryParams.append('agentRef', filters.agentRef);
        }
        ...
        const response = await axios.get(
          `https://abroad-backend-gray.vercel.app/api/payment-tagging/admin/getall?${queryParams}`,
  ...
  useEffect(() => {
    saveFiltersToStorage(FILTER_STORAGE_KEYS.ADMIN_PAYMENT_TAGGING, filters);
  }, [filters]);
  ...
  const handleClearFilters = () => {
    ...
    clearFiltersFromStorage(FILTER_STORAGE_KEYS.ADMIN_PAYMENT_TAGGING);
    setPagination((prev) => ({ ...prev, page: 1 }));
  };
  ...
  const handleExportToExcel = () => {
    ...
    XLSX.writeFile(wb, 'admin_payment_tagging_records.xlsx');
  };
              ...
              <Grid item xs={12}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    label="Agent"
                    onChange={(e) =>
                      handleFilterChange('agentRef', e.target.value)
                    }
                  >
...
export default AdminPaymentTagging;
